// import numbersArray from "./public/admin"
...
var starting_throughput = 5;
// var customer_demand = [4, 8, 12, 16, 20];
var customer_demand ;

// This controls how the roles are labeled
var NOOSHABE_NAMES = ["خرده فروش", "عمده فروش", "توزیع کننده", "کارخانه"];
// This is what stores all the role data during the game
var ROLE_0 = {
    "name": NOOSHABE_NAMES[0],
    "upstream": {
        "name": NOOSHABE_NAMES[1],
...
var NOOSHABE_ROLES = [ROLE_0, ROLE_1, ROLE_2, ROLE_3];
...
io.on('connection', function (socket) {
    ...
    socket.on('submit password', function (msg, callback) {
        ...
        if (msg == "admin") {
            ...
            callback({ status: gameStatus, numUsers: numUsers, groups: groups , starting_throughput: starting_throughput });
        } else {
    ...
    socket.on('start game', function (msg , callback) {
        var canStart = true;
        var gameEnded = false;
        if (gameStarted) {
        
            return callback({ err: "بازی از قبل شروع شده است." });
        } else {
            if (numUsers == 0) {
                return callback({ err: "برای انجام بازی به حداقل 4 نفر نیاز دارید." });
            } else if (numUsers % 4 != 0) {
                return callback({ err: "قبل از شروع بازی باید هر گروه را پر کنید." });
            }

            gameStarted = true;
            callback({ numUsers: numUsers });
            customer_demand = msg.numbersArray;
            inventory_cost = msg.inventorycost 
            backlog_cost= msg.backlogcost
...
http.listen(process.env.PORT || 3000, function () {
    console.log('Nooshabe Distribution Game Simulator engaged! Listening...');
});
...
function advanceTurn(group) {
    ...
    if (groupToAdvance.week == 0) {
        groupToAdvance.waitingForOrders = NOOSHABE_NAMES;
    ...
    for (var i = 0; i < groupToAdvance.users.length; i++) {
        ...
        if (i == 0) {
            ...
            curUser.role.downstream.PrevOrders = groupToAdvance.mailing[i - 1];
    ...
    // Next week
    groupToAdvance.week++;
    groupToAdvance.waitingForOrders = JSON.parse(JSON.stringify(NOOSHABE_NAMES));
...
function resetGame() {
    for (var i = 0; i < groups.length; i++) {
        ...
        roles = JSON.parse(JSON.stringify(NOOSHABE_ROLES));
        // Reset all the users
        for (var j = 0; j < groups[i].users.length; j++) {
...
function registerUser(socketId, userName) {
    ...
    if (roles.length == 0) roles = JSON.parse(JSON.stringify(NOOSHABE_ROLES));
